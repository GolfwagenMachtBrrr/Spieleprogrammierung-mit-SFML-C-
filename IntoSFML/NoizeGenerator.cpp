...
std::vector<float> NoizeGenerator::Generate(const int &width, const int &height, const float &scale, const std::vector<Wave> &waves, const sf::Vector2f &offset)
{
    std::vector<float> noiseMap(width*height, 0);
    ...
    for (int i = 0; i < width; i++)
    {
        for (int j = 0; j < height; j++)
        {
            ...
            float samplePosX = static_cast<float>(i) * scale + offset.x; 
            float samplePosY = static_cast<float>(j) * scale + offset.y;
